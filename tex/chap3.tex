...
Since the CS should only consist of brief user interaction and be as automated as possible, desktop and web applications are not valid as tools for conversion. This excludes \emph{Deep Zoom Composer}, \emph{MapTiler}, \emph{OpenSeaDragon DZI Online Composer} and \emph{Zoomable} as possible choices (therefore also excluding (next to the reasons given in section 2.1.3), TMS as possible format).
...
The second iteration uses VIPS python implementation, which is not only faster than deepzoom.py \review{(citation or data needed)}, but also capable of converting all the given test images.
