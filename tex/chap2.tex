...
\review{massive und enormous sind übertreibend}

Due to the amount of data stored in a raw, uncompressed WSI\footnote{ A typical 1,600 megapixel slide requires about 4.6 GB of memory on average\cite{Farahanil15}. The size of a H\&E (hematoxylin and eosin) stained slide ranges typically from 4 to 20 GB\cite{Singh11}.}, file formatting and compression is required to make working with WSIs feasible. Since there is no standardized format for WSIs, vendors came up with their own, proprietary solutions, which vary greatly\cite{Cornish13}. Efforts of standardization are being made through the \emph{Digital Imaging and Communications in Medicine} (DICOM) Standard\cite{DICOM10}\nmc{DICOM}{Digital Imaging and Communications in Medicine}.

Usually, WSI files are stored as a multitude of single images, spanning multiple folders and different resolutions. Those files are used to construct a so called \emph{image pyramid}\cite{Farahanil15} (see fig. 2.1 and subsection \ref{sec:dicomSupplement}).
...
Slides with multiple regions of interest are structured as a single image pyramid enclosing all regions. Slides may omit pixel data for TIFF tiles not in an ROI \review{Was heisst ROI ausgeschrieben?}. When such tiles are downsampled into a tile that does contain pixel data, their contents are rendered as white pixels\cite{web:openslide}.
...
The Deep Zoom Image (DZI)\nomenclature{DZI}{Deep Zoom Image} format is an XML-based file format, developed and maintained by Microsoft\cite{web:dzi}. A DZI is a pyramidcal, tiled image (see fig. 2.2), similar to the one described in 2.1.1 (compare fig. 2.1 and 2.2), with two exceptions:
\begin{enumerate}
...
Both, OSM and TMS, offer zooming images, which in general, have the functionality necessary, to be of use for this thesis. Unfortunately, they are also highly specialized on the needs of the mapping community, which makes them unattractive choices for the research objective of this thesis respectively.
\review{unattractive ist imho nicht so gut ausgedrückt. Ich würde eher was in die richtung: "Unfortunately, they are also highly specialized on the needs of the mapping community, with many features not needed in the context of this thesis.}
...
TIFF and BigTIFF are capable of saving images in multiple resolutions. Together with the feature of saving tiles, the image pyramid model described in 2.1.1 can be taken advantage of\cite{web:digitalpreservation2}. \review{Den letzten Satz würde ich nochmal umformulieren: Together with the feature of saving tiles, an image pyramid model (as described in 2.1.1) can be applied oder so}


\section{Short Introduction to Neural Networks}
\review{Auch hier wieder das Problem: steht NN für ein Neural Network oder für mehrere?}
The objective of the workflows introduced in chapter 1.2. is to create training samples for NN. Before going into other details, it is necessary to clarify what NN are, how they work, why they need training samples and what they use them for\footnote{
...
One of the biggest strengths of a NN, much like a BNN, is the ability to adapt by learning\footnote{As humans, NN learn by training\cite{Shiffman12}.}. This adaption is based on \emph{weights} that are assigned to the connections between single neurons. Fig 2.7 shows an exemplary NN with neurons and the connections between them.
...
The perceptron was invented by Rosenblatt at the Cornell Aeronautical Laboratory in 1957\cite{Rosenblatt58}. It is the computational model of a single neuron and as such, the simplest NN possible\cite{Shiffman12}. A perceptron consists of one or more inputs, a processor and a single output (see fig. 2.8)\cite{Rosenblatt58}.
...
The following section elaborates on the concept of \emph{Microservices} (MS)\nmc{MS}{Microservice}, defining what they are, listing their pros and cons, as well as explaining why this approach was chosen over a monolithic approach \review{eigentlich hat die erklärung warum du microservices genutzt hast in den grundlagen nichts verloren :p} . A monolithic software solution is described by \cite{Lewis14} as follows:
\begin{quotation}
...
The tools represented in 1.2 were implemented by using the MS modularization patterns. Due to the implementation being done by a single person, some of the inherent disadvantages of MS are negated (making them a favorable modularization concept):
\begin{itemize}
...
\begin{itemize}
	...
	\item separating the steps of the process chain into multiple MS makes for a smaller and easier maintainable code base \review{whait what? im Vergleich zu nem Monolithen? Glaube ich nicht :p Ich glaube was du sagen willst ist: separating the steps of the process chain into multiple MS leads to well separated modules, each having a small and therefore easy to maintain codebase}
	...
	\item the implementation of the project will happen in an iterative, continuous manner, which is easily doable with the use of MS \review{würde ich eventuell als punkt rausnehmen weil für die arbeit nicht relevant}
\end{itemize}
...
\review{wenn du cool bist machst du hier noch ein process chart als bild rein der deinen gesamtprozess beschreibt, UML fuckyeah :p (alternativ auch in den anhang, falls dir das zu dick aufgetragen ist)}
...
The devices which create WSIs, so called \emph{whole slide scanners}, create images in various formats, depending on the producer system (due to the lack of standardization \cite{Cornish13}). The Conversion Service (CS)\nmc{CS}{Conversion Service} has the goal of converting those formats to an open format\footnote{see chap. 2.1.2 for the reasons} (see fig. 2.12).
...
As mentioned in 2.4, the Annotation Service (AS)\nmc{AS}{Annotation Service} will provide a graphical user interface (GUI)\nmc{GUI}{Graphical User Interface} to view a WSI, create annotations and manage those annotations. This also includes persisting made annotations in a file (see fig. 2.13).
...
Let there be a WSI $Img$ and a corresponding set of annotations $A$. The TS will achieve the extraction by iterating over every $a_i \in A$, creating a sub-image $img_i$ which is the smallest bounding box around the region described by $a_i$ (see fig. 2.14). To be used as training sample, the TS must keep up the relationship between $img_i$ and $a_i$.