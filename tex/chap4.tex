...
The two components interact as follows: once the client requested a valid image URL, the ASS will check if the requested WSI is a DZI and, if so, render a ASV with the image path, MPP \review{Was ist MPP?} and file name of the WSI. If the WSI is proprietary, it will be wrapped by OpenSlide. The remaining procedure is then identical to the DZI case.

The ASV is returned to the client and requests the data necessary to view the WSI and it's annotations (this includes configurations, previously made annotations if present, labels and the image tiles for the current view).
\review{Hier eventuell eher was in die Richtung: "The ASV is served to the client as a web application that requests the data \dots" oder "the ASV code is executed by the client and requests the data\dots}
...
} generates all data necessary, to work with a proprietary WSI as if it would be a DZI\cite{web:openslide}. The \texttt{get{\textunderscore}dzi(format)} and \texttt{get{\textunderscore}tile(level, address)} functions are of special importance:

\review{Woher kommen diese beiden Funktionen? Sind das Teile der DZG Lib? Wenn ja, explizit reinschreiben}
...
\review{Eventuell: The OSDV provides a function called \texttt{zoomPerClick} that enhances the image section under the cursor. Since drawing...}
...
\review{Fußnote nach oben ziehen: supported by the OSDV implement a tiled image pyramid scheme (see \ref{sec:dicomSupplement}). Thus, a number of images must be opened. This functionality is provided by the \texttt{open(tileSources)} function...}
...
\label{sec4_region}
\review{Der Absatz hier holpert imho noch ziemlich. Eventuell umformulieren in richtung: The basic concept used to realize annotations in the ASV are regions \dots Außerdem eine der beiden referenzen auf tab. \ref{tab4_regionUML} rausnehnmen.}
...
The \textbf{name} property holds the title of the region's corresponding label. A region is created with the title of the label selected \review{selected wo?} at the time. The name property cannot be changed, unless the whole region object is deleted and redrawn with another corresponding label selected (prior to the redrawing).
