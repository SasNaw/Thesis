...
\begin{itemize}
	\item Flask\cite{web:flask} (see chap. 4.3.1)
	\item OpenSlide Python\cite{web:openslide} (see chap. 4.3.2)
\end{itemize}
...
Through the use of decorators a RESTful API was deployed for the ASS (compare chap. 4.3.3).
...
To communicate with the ASS a RESTful API was deployed. This is realized with Flasks route() decorators\footnote{see chap. 4.3.1}. The listing below gives an overview over the URLs that the ASS RESTful API offers (in the style of \textbf{URL} (method): \emph{function(parameters)}), followed by a brief description of the functionality\footnote{see Appendix B.1 for a detailed documentation of the ASS functions}:
