...
\texttt{toggleRegions(uid)} turns all regions of the corresponding label (in-)visible. If one of the regions to be toggled is currently selected it is deselected (via \texttt{deselectRegion(reg)}, see fig. \ref{figB_singleClickLabel}).
...
\texttt{deselectRegion(reg)} deselects the provided region \emph{reg}.
